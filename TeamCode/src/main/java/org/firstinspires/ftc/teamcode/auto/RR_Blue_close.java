...
@Config
@Autonomous(name="RR_Blue_close",group = "Auto")
public class RR_Blue_close extends LinearOpMode {
    public static double Final_angle = 0; //the drop pose angle adjust - use it to make sure the robot faces the board straightly.
    public static double end_pose1_y_adj =0; //the first drop y adjust (positive - move left)
    public static double end_pose1_x_adj= 0; //the first drop x adjust (positive - close to board)
    public static double end_pose_Y_i = 35.5;
    double end_pose_Y = 0 ;
    ...
    private int Tagid = 1;  //for the AprilTag looking
    ...
    public void runOpMode () throws InterruptedException {
        ...
        //Blue close: (12, 62.75, Math.toRadians(90))
        Pose2d Blue_close_pose = new Pose2d(12, 62.75, Math.toRadians(90));
        Pose2d End_2nd = new Pose2d(0, 0, Math.toRadians(0));
        drive.setPoseEstimate(Blue_close_pose);
        //trajectory for left target (1)
        //1st trajectory, spline (back) and turn 89 degree (clockwise) to put the pixel on left line
        Trajectory traj1L = drive.trajectoryBuilder(Blue_close_pose,true)
        ...
        Trajectory traj2L = drive.trajectoryBuilder(traj1L.end())
                .splineTo(new Vector2d(8,58),Math.toRadians(135)) //8,58
                .build();

        //trajectory for middle target(2)
        //1st trajectory, move back to put the pixel on the center line
        Trajectory traj1M = drive.trajectoryBuilder(Blue_close_pose)
                .back(30.5)
                .build();
        ...
        Trajectory traj1R = drive.trajectoryBuilder(Blue_close_pose,true)
        ...
        Trajectory traj2R = drive.trajectoryBuilder(traj1R.end())
                .lineToSplineHeading(new Pose2d(15,50,Math.toRadians(90))) //13,50
                .build();
        waitForStart();
        ...
        if (isStarted()) {
            ...
            //score purple pixel (forward and back)
            switch (side) {
                case 1: //left
    ...
    private void initAprilTag() {
        ...
        blueDetectionLeft = new BlueDetectionLeft(telemetry);
